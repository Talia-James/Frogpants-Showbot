...
def locate_livestream_link(channel_url):
    driver = build_headless()
    driver.get(channel_url)
...
def find_stream_id(channel_name):
    ...
    live = detect_stream(channel_url)
    ...
    if live:
        livestream_link = locate_livestream_link(channel_url) #Once a stream is confirmed live, use the channel HTML link to find the link to the currently live stream
...
#TODO: The Selenium functions have some recursions and redundencies that should be streamlined. For one, the driver could be kept open if the stream
#is detected to be live, rather than opening it in two different function calls. Could likely chain detect_stream() as a container then build_headless(),
#check for the live stream triggers, pass False if not detected or if detected, chain locate_livestream_link() into find_stream_id()
#tl;dr: there's no real reason to separate out a lot of these functions into two Selenium driver calls.
...
def build_credentials(client_secrets_file,scopes):  
...
def build_yt_obj(module,credentials,api_service_name=api_service_name,api_version=api_version):
...
author = 'AUTHOR_VAR'
randomized_messages = [
    f"[Bot response] Title submitted, {author}, you're doing a great job!",
    f'[Bot response] I see (and submit) what you did there, {author}.',
    f'[Bot response] Titles are like a box of chocolates, and {author} never knows...this is the dumbest metaphor ever',
    f"[Bot response] {author} wrote a cool title, but it's no FARTGAS.",
    f"[Bot response] I can definitely see why you like that, {author}",
    f"[Bot response] {author} is testing the ship's phasers with that title.",
    f'[Bot response] Great title, {author}. I love you. But not in a weird way.',
    f'[Bot response] Thanks for training my AI, {author}. I will remember you when us bots take over!',
    f'[Bot response] Showbot is ready to receive your limp submission, {author}',
    f'[Bot response] Mendoza got your submission, {author}. Expect a response...soon.',
    f'[Bot response] Your submission has now been sent to The Uath, {author}. In the clip clop.',
    f'[Bot response] Thanks for being a damn distracting freak, {author}.',
    f"[Bot response] Did you know your submission is not automatically upvoted? Vote for yourself, {author}, it's totally not masturbation!",
    f'[Bot response] Your submission is never too early for a fish sandwich, {author}.',
    f"[Bot response] Title submitted, {author}. Fert!"
    ]
...
#TODO: The credentials variable could possible be saved and passed within the script and might not need to be rebuilt each time.
